...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	var coverOpening:FlxSprite;
	var bgwall:FNFSprite;
	var lockers:FNFSprite;
	var bgchar1:FNFSprite;
	var bgchar2:FNFSprite;
	var bgchar3:FNFSprite;

	public var supJew:FNFSprite;

	var paunch:FNFSprite;
	var kennyDies:FNFSprite;
	var crowdShock:FNFSprite;
	var stanLine:FlxSprite; //stanline is flxsprite because the animation doesnt play in fnfsprite
	...
	public function new(curStage)
	{
		...
		if (PlayState.determinedChartType == "FNF")
		{
			...
			{
				case 'lafs' | 'sup-jew' | 'ten-percent-off':
					curStage = 'southSchool';
				default:
					curStage = 'stage';
			}
		...
		switch (curStage)
		{
			...
			case 'southSchool':
				...
				var school_tex = Paths.getSparrowAtlas('backgrounds/$curStage/school_assets');
				
				coverOpening = new FlxSprite(890, 570).makeGraphic(500, 500, 0xFFCF984F);
				coverOpening.scrollFactor.set(0.9, 1);
				add(coverOpening);

				bgwall = new FNFSprite(630, -70);
				bgwall.frames = school_tex;
				bgwall.animation.addByPrefix('idle', 'bg', 24);
				bgwall.animation.play('idle');
				bgwall.setGraphicSize(Std.int(bgwall.width * 1.1));
				bgwall.scrollFactor.set(0.9, 1);
				add(bgwall);
				bgwall.animation.play('idle');
					
				lockers = new FNFSprite( -466, 40);
				lockers.frames = school_tex;
				lockers.animation.addByPrefix('idle', 'locker', 24);
				lockers.animation.play('idle');
				lockers.setGraphicSize(Std.int(lockers.width * 2.2));
				lockers.scrollFactor.set(1, 1);
				add(lockers);
				lockers.animation.play('idle');
					
				bgchar1 = new FNFSprite(360, -30);
				bgchar1.frames = school_tex;
				bgchar1.setGraphicSize(Std.int(bgchar1.width * 0.8));
				bgchar1.animation.addByPrefix('idle', 'backcrowdbop', 24, false);
				bgchar1.scrollFactor.set(0.9, 1);
				add(bgchar1);
					
				bgchar2 = new FNFSprite(400, 150);
				bgchar2.frames = school_tex;
				bgchar2.setGraphicSize(Std.int(bgchar2.width * 0.8));
				bgchar2.animation.addByPrefix('idle', 'midcrowdbop', 24, false);
				bgchar2.scrollFactor.set(0.95, 1);
				add(bgchar2);
					
				bgchar3 = new FNFSprite(940, 260);
				bgchar3.frames = school_tex;
				bgchar3.setGraphicSize(Std.int(bgchar3.width * 0.8));
				bgchar3.animation.addByPrefix('idle', 'dogpoobop', 24, false);
				bgchar3.scrollFactor.set(1, 1);
				add(bgchar3);

				//cutscenes damn you jay

				//last minute sup jew addition aaaaaaa
				supJew = new FNFSprite(480, 220);
				supJew.frames = Paths.getSparrowAtlas('cutscene/supjew/supjew-text');
				supJew.animation.addByPrefix('appear', 'supjew0', 24, false);
				supJew.animation.play('appear');
				supJew.visible = false;
				foreground.add(supJew);

				paunch = new FNFSprite(-615, 60);
				paunch.frames = Paths.getSparrowAtlas('cutscene/punch/CartmanPunch');
				paunch.setGraphicSize(Std.int(paunch.width * 0.95));
				paunch.animation.addByPrefix('punch', 'punch instance 1', 24, false);
				paunch.visible = false;
				paunch.animation.play('punch');
				foreground.add(paunch);

				kennyDies = new FNFSprite(598, -418);
				kennyDies.frames = Paths.getSparrowAtlas('cutscene/punch/KennyDies');
				kennyDies.setGraphicSize(Std.int(kennyDies.width * 0.8));
				kennyDies.animation.addByPrefix('die', 'anim instance 1', 24, false);
				kennyDies.scrollFactor.set(0.95, 1);
				kennyDies.visible = false;
				kennyDies.animation.play('die');
				
				stanLine = new FlxSprite(667, 192);
				stanLine.frames = Paths.getSparrowAtlas('cutscene/punch/IconicLine');
				stanLine.setGraphicSize(Std.int(stanLine.width * 0.8));
				stanLine.animation.addByPrefix('omg', 'oh my gah', 24, false);
				stanLine.scrollFactor.set(0.95, 1);
				stanLine.visible = false;
				stanLine.animation.play('omg', true);

				crowdShock = new FNFSprite(348, -34);
				crowdShock.frames = Paths.getSparrowAtlas('cutscene/punch/CrowdShock');
				crowdShock.setGraphicSize(Std.int(crowdShock.width * 0.8));
				crowdShock.animation.addByPrefix('shock', 'anim instance 1', 24, false);
				crowdShock.scrollFactor.set(0.95, 1);
				crowdShock.visible = false;
				crowdShock.animation.play('shock');
				foreground.add(crowdShock);
				foreground.add(stanLine);
				foreground.add(kennyDies);
	...
	public function stageUpdate(curBeat:Int, boyfriend:Boyfriend, gf:Character, dadOpponent:Character)
	{
		...
		switch (PlayState.curStage)
		{
			...
			case 'southSchool':
				bgchar1.animation.play('idle');
				bgchar2.animation.play('idle');
				bgchar3.animation.play('idle');

				if (CoolUtil.spaceToDash(PlayState.SONG.song.toLowerCase()) == 'ten-percent-off') {
					if (curBeat == 450) {
						PlayState.dadOpponent.visible = false;
						PlayState.boyfriend.visible = false;
						paunch.visible = true;
						paunch.animation.play('punch', true);
					}
				}

				if (CoolUtil.spaceToDash(PlayState.SONG.song.toLowerCase()) == 'sup-jew') {
					if (curBeat == 262) {
						supJew.visible = true;
						supJew.playAnim('appear');
					}
				}
		}
	}

	var punchPlayed:Bool = false;
	var deathPlayed:Bool = false;
	var kennyPlayed:Bool = false;
	...
	public function stageUpdateConstant(elapsed:Float, boyfriend:Boyfriend, gf:Character, dadOpponent:Character)
	{
		switch (PlayState.curStage)
		{
			...
			case 'southSchool':
				if (supJew.animation.curAnim.finished && supJew.visible)
					if (supJew.visible)
						supJew.visible = false;

				if (paunch.animation.curAnim.curFrame == 9 && paunch.visible) {
					kennyDies.visible = true;
					crowdShock.visible = true;
					FlxG.camera.shake(0.02, 0.2);
					if (!punchPlayed) {
						FlxG.sound.play(Paths.sound('PAUNCH'));
						punchPlayed = true;
					}
					if (!deathPlayed) {
					...
					kennyDies.animation.play('die', true);
					crowdShock.animation.play('shock', true);
					crowdShock.animation.curAnim.curFrame = 0;
					bgchar3.visible = false;
					bgchar2.visible = false;
					bgchar1.visible = false;
					FlxG.camera.follow(PlayState.camKenny, null, Main.framerateAdjust(0.10));
					PlayState.defaultCamZoom += 0.02;
				}

				if (kennyDies.animation.curAnim.curFrame == 20 && kennyDies.visible) {
					FlxG.camera.shake(0.02, 0.4);
					new FlxTimer().start(1.2, function(timer:FlxTimer) {
						stanLine.visible = true;
						stanLine.animation.play('omg', true);
						if (!kennyPlayed) {
							FlxG.sound.play(Paths.sound('TheyKilledKenny'));
							kennyPlayed = true;
						}
					});
				}
