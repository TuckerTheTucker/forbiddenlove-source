...
class PlayState extends MusicBeatState
{
	...
	public static var camKenny:FlxObject;
	...
	override public function create()
	{
		...
		camPos.set(FlxG.width / 2 - 200, FlxG.height / 2);
		...
		camKenny = new FlxObject(728, 322, 1, 1);
		camKenny.setPosition(camPos.x, camPos.y);
	...
	override function beatHit()
	{
		...
		if (CoolUtil.spaceToDash(SONG.song.toLowerCase()) == 'ten-percent-off') {
			if (curBeat == 450)
				canReset = false;
		}
	...
	private function callDefaultSongEnd()
	{
		...
		difficulty = difficulty.replace('-weak', '');
	...
	private function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			{
				case 0:
					FlxTween.tween(ClassHUD.bfSideText, {y: ClassHUD.bfSideText.y + 10, alpha: 1}, 1, {ease: FlxEase.circOut});
					FlxTween.tween(ClassHUD.dadSideText, {y: ClassHUD.dadSideText.y + 10, alpha: 1}, 1, {ease: FlxEase.circOut, startDelay: 0.4});
					new FlxTimer().start(5, function(tmr:FlxTimer)
					{
						FlxTween.tween(ClassHUD.bfSideText, {y: ClassHUD.bfSideText.y - 10, alpha: 0}, 1, {ease: FlxEase.circOut});
						FlxTween.tween(ClassHUD.dadSideText, {y: ClassHUD.dadSideText.y - 10, alpha: 0}, 1, {ease: FlxEase.circOut});
					});
